...
with open(snakemake.input.mapping_list, 'r') as mapping_list, open(snakemake.output.bam_list,'w') as bam_list:
    for line in mapping_list:
        ...
        shell(f"minimap2 -t {snakemake.threads} {snakemake.params.extra} {fasta} {fastq} | samtools view -Sbh | samtools sort - -o {bam}")
...
print('Merging')
shell(f"samtools merge -b {snakemake.output.bam_list} -h {snakemake.input.mapping_dir}/header.tmp | samtools sort - -o {snakemake.output.bam} ;")
shell(f"samtools index {snakemake.output.bam}")
shell(f"rm {snakemake.input.mapping_dir}/header.tmp")
